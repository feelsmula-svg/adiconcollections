...
export function buildInvoicePdf(order: OrderDetail): jsPDF {
...
/**
 * Server-side helper: render the invoice PDF to a Node Buffer (e.g. to attach
 * to a transactional email). Uses jsPDF's arraybuffer output — no DOM needed,
 * since the invoice draws with text/line primitives only.
 */
export function renderInvoicePdfBuffer(order: OrderDetail): Buffer {
  const doc = buildInvoicePdf(order);
  return Buffer.from(doc.output("arraybuffer"));
}

/** Canonical, filesystem-safe invoice filename for an order. */
export function invoiceFileName(order: OrderDetail): string {
  return `invoice-${order.reference.replace(/[^A-Za-z0-9-]+/g, "")}.pdf`;
}
