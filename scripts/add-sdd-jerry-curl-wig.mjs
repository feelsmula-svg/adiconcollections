// Insert the SDD Jerry Curl Wig (100% raw human hair) into MongoDB. Two length
// variants: 18" $250, 20" $280. Image embedded as base64 data URL. Idempotent
// — skips if a product with the same `name` already exists.
//
// Usage:
//   node scripts/add-sdd-jerry-curl-wig.mjs
...
const PRODUCT = {
  name: "SDD Jerry Curl Wig",
  description: "100% Raw Human Hair",
  category: "wigs",
  type: "SDD Jerry Curl",
  // Default unit price = lowest length (18").
  priceCents: 25_000,
  imagePaths: ["/products/sdd-jerry-curl-wig-a.jpeg"],
  stock: 10,
  featured: true,
